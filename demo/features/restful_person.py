...
@step(u'When I \'([^\']*)\' to the \'([^\']*)\' resource url')
...
@step(u"And I '([^']*)' to the '([^']*)' resource url with the '([^']*)' from the previous response")
...
@step(u"And I '([^']*)' to the '([^']*)' resource url with the '([^']*)' from the previous response with the following data")
def verb_to_url_with_attribute_from_previous_response(step, verb, url, attribute):
    world.browser = Client()
    url_method = getattr(world.browser, verb.lower())
    try:
